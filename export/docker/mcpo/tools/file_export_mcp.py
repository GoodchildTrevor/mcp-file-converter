...
def search_local_flux(query: str):
    log.debug(f"Searching for local SD image with query: '{query}'")
    FLUX_URL = os.getenv("LOCAL_FLUX_URL")
    DEFAULT_STEPS = int(os.getenv("LOCAL_FLUX_STEPS", 5))
    DEFAULT_WIDTH = int(os.getenv("LOCAL_FLUX_WIDTH", 512))
    DEFAULT_HEIGHT = int(os.getenv("LOCAL_FLUX_HEIGHT", 512))
    DEFAULT_SCALE = float(os.getenv("LOCAL_FLUX_SCALE", 1.0))
    DEFAUL_TIMEOUT = int(os.getenv("LOCAL_FLUX_TIMEOUT", 30))

    if not FLUX_URL:
        log.warning("LOCAL_FLUX_URL is not defined.")
        return None
    payload = {
        "prompt": query.strip(),
